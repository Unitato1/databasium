    ...
    class Schemas::HeaderActions < Components::Base
      ...
      def view_template
        div(id: "header_actions", class: "flex flex gap-2") do
          ...
          form_with(method: :get, url: model_layers_path(model, layers), class: "border-l border-border pl-2 inline-flex gap-2 items-center") do |form|
            hidden_field_tag :model, model
            form.number_field :layers, value: layers, class: "w-10 border-1 border-border rounded-xl p-1 text-center text-sm"
            form.submit "Search for associations", class: "text-sm bg-accent shadow-accent rounded-xl p-1.5 text-center"
          end
        end
      end
