...
export default class extends Controller {
  ...
  createSeperator() {
    ...
    seperator.classList = "bg-panel inline-block h-2 w-3";
    return seperator;
  }
  ...
  chooseColumn(event) {
    ...
    if (this.previousValues.get(event.target)) {
      while (
        ...
        event.target.nextElementSibling.tagName !== "BUTTON"
      ) {
  ...
  addFilter(e, withOperatorType = true) {
    ...
    if (withOperatorType) {
      containerDiv.appendChild(this.createOperatorTypeField());
    } else {
      containerDiv.appendChild(this.createWhereField());
    }
    containerDiv.appendChild(this.createSeperator());
  ...
  createInputField(selectedAttribute) {
    ...
    inputField.classList = "px-4 py-2 rounded-md border-1 border-border w-fit max-w-40 h-10";
  ...
  createWhereField() {
    const whereField = document.createElement("select");
    whereField.classList = "px-4 py-2 rounded-md border-1 border-border w-fit h-10";
    whereField.add(new Option("Where", "Where"));
    whereField.disabled = true;
    return whereField;
  }
